...
- **`n_2 / n_3 / n_15` relation typs — correctness validation** — first-pass implementations are active in the solver (`n_2` hard same-period; `n_3` + `n_15` soft alternate-day / even-spacing) based on reasoned interpretations. Full correctness validation waits on a real sample XML showing these typs in the wild.
- **Print template body review** — the 24-template registry is loaded into the previewer, but the body of each newer template hasn't had a side-by-side audit against the baseline equivalent. Visual parity may still drift cell-by-cell.
...
1. **Legacy dark-navy welcome header was rendering inside the new shell** — the shell wrapped existing body content into `chrx-main`, but didn't hide the old "Chronexa / Open timetable. Yours to keep." dark-gradient header bar. Result: a 2012-Classic-blue strip jammed in the middle of the editorial-paper layout, plus duplicate Test/Generate buttons.
...
- **Design v2 ("Studio")** shipped first as `css/design-v2.css`. Inter Tight + JetBrains Mono, warm-grey neutrals, electric-indigo accent used sparingly, layered-shadow card ladder, translucent ribbons with `backdrop-filter: blur(24px)`. Anti-references: Classic 2012 / Cloud UI / Office Ribbon / Material defaults. Influences: Linear, Vercel, Notion, Things 3, Raycast. Documented at `Chronexa-DESIGN.md`.
...
**Headline: warm-start places 946/0 conflicts on sample-school.xml — Chronexa FEASIBLE, +2 placements over Classic, 0 hard violations vs Classic's 2.** Two one-line bugs in `csp_solver.js#buildModel` were the entire story:
...
## 📝 Prior session (2026-05-21, third) — Chronexa first beat Classic — initial groupIds fix

**Headline: the warm-start now places 946 / 5 conflicts versus Classic's 944 / 7 — Chronexa is +2 placements better than Classic on its own XML.** The win came from a single one-line fix in `js/solver/csp_solver.js#buildModel`: the per-card expansion was dropping `groupIds`, so every lesson with an elective group (URDU, SANSKRIT, Music, Dance, etc.) was being treated as a whole-class lesson under group-aware conflict detection. Result: 28 of the 35 previously-unplaceable URDU electives now schedule cleanly alongside their SANSKRIT counterparts in the same slot for the same class, the way Classic has always done. Cold-path also lifted: median 877 → 905 placements.
...
**Wine cleanup.** The Wine setup from the earlier soft-rel session (~2 GB across `/Applications/Wine Stable.app`, `~/.wine-classic/`, and the Homebrew cache tarball) was removed once the legacy column of `docs/SOLVER_VS_LEGACY.md` was filled via XML evaluation instead. MemPalace `wing_user/wine-works-on-m3-via-cache-extraction` keeps the cache-extract methodology documented; `wing_user/wine-removed-2026-05-21` records the cleanup.
...
**Cross-harness for legacy comparison is in place and the legacy column is filled.** Headless Node runner (`tools/run_baseline.mjs`) loads `sample-school.xml` through the existing browser-side XML parser via jsdom, drives `solve()` across N seeds in cold and warm-start modes, and emits a Markdown-ready table. A focused bias test (`tools/test_bias.mjs`) discriminates the soft-rel hookup. The legacy column was filled the right way: `sample-school.xml` is itself an aSc TimeTables export, so the 951 `<card>` entries in it already are Classic's solver output. A new evaluator (`tools/evaluate_classic.mjs`) replays Classic's placement through Chronexa's hard-rule filter + soft-scorer in 100 ms — no Wine, no roz.exe needed. **Headline finding:** Chronexa's warm-start reaches Classic's exact numbers (916 placed / 35 conflicts / −4,950 soft) at `t = 75 ms` and the next 15 seconds of search neither improve nor degrade them; verified by `tools/warm_trajectory.mjs` with the solver's `onProgress` callback. Chronexa's cold-path loses to Classic by ~40 placements / 39 conflicts on this dense GD Goenka fixture. Beating Classic on its own XML would require a different search strategy (LNS / SA on the warm state) or relaxed hard rules. The Wine-via-Homebrew-cache setup is still installed (`/Applications/Wine Stable.app/` + `~/.wine-classic/`) for the optional path of reading Classic's *self-reported* numbers; it's documented at the bottom of `docs/SOLVER_VS_LEGACY.md` but not load-bearing — the placement / conflict / soft comparison ships without it.
