"""Python: placement + teacher + class only (no soft, no hard cap, no rooms)"""
import json
...
for L in school["lessons"]:
    ...
    for _ in range(ncards):
        cards.append({"length": length, "teachers": teachers, "classes": classes, "cls_occ": cls_occ, "rooms": []})
...
for ci, card in enumerate(cards):
    ...
    for s in slots:
        if card["length"] == 2 and pidx[s[1]] + 1 >= len(periods):
            continue
        av[s] = m.NewBoolVar(f"a_{ci}_{slot_index[s]}")
    avars[ci] = av
    pl = m.NewBoolVar(f"p_{ci}")
    m.Add(sum(av.values()) == pl)
    placed.append(pl)

t_occ = defaultdict(list)
cls_at = defaultdict(list)
for ci, card in enumerate(cards):
    for s, avar in avars[ci].items():
        d, p = s
        covered = [(d, p)] if card["length"] == 1 else [(d, p), (d, periods[pidx[p] + 1])]
        for ps in covered:
...
print(f"PYTHON (placement+teacher+class, no soft/rooms): status={s.StatusName(status)} placed={n_placed}/{len(cards)} wall={s.WallTime():.2f}s")
