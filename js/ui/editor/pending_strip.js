...
window.PendingStrip = (function () {
  ...
  function buildGroups(S, placedCounts) {
    ...
    // Week/term view filter (lane W3b-7): the tray lists what still has to be
    // placed IN THE VIEW YOU ASKED FOR — a Week-B lesson is not an unplaced card
    // while Week A is on screen.
    const viewFilter = window.ViewFilter ? window.ViewFilter.currentFilter() : { week: "all", term: "all" };

   for (const L of (S.lessons || [])) {
     if (classFilterId && !(L.classIds || []).includes(classFilterId)) continue;
     if (!lessonMatchesFilter(L, viewFilter, S)) continue;
