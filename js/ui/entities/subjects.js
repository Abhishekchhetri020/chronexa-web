...
import "../components/time_off_matrix.js";
...
(function (global) {
  ...
  function rows() {
    ...
    const cfList = (window.CustomFields && window.CustomFields.getFields(window.APP?.school, "subjects")) || [];
    return ((window.APP.school && window.APP.school.subjects) || []).map(s => {
      ...
      cfList.forEach(cf => {
        rowObj[cf.id] = (s.customValues && s.customValues[cf.id] != null) ? s.customValues[cf.id] : "";
      });
      return rowObj;
    });
  }
  ...
  function columns() {
    ...
    if (window.CustomFields) {
      cols.push(...window.CustomFields.getCustomColumns("subjects"));
    }
    return cols;
  }
  ...
  function openEdit(r) {
    ...
    function save() {
      ...
      const ns = isNew ? { id:D.uid("s"), name:draft.name.trim(),
        ...
        customValues: Object.keys(draftCustomValues).length ? { ...draftCustomValues } : undefined } : null;
      ...
      window.APP.mutate(isNew ? "Add subject" : "Edit subject", (school) => {
        if (!isNew) {
          ...
          subj.customValues = Object.keys(draftCustomValues).length ? { ...draftCustomValues } : undefined;
    ...
    const cfInputs = (window.CustomFields && window.CustomFields.buildEditFields("subjects", draftCustomValues)) || [];
  ...
  function open(opts) {
    ...
    D.open({
      ...
      onAction: (cmd, row) => {
        ...
        if (cmd === "customfields") {
          if (window.CustomFields) {
            window.CustomFields.open({ entity: "subjects", onUpdate: () => D.refresh(rows(), columns()) });
          }
          return;
        }
        if (cmd === "delete" && row) {
...
export const EntitySubjects = window.EntitySubjects;
