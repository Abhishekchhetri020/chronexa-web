...
import "../components/class_constraints_dialog.js";
import "../components/divisions_tree.js";
...
(function (global) {
  ...
  function rows() {
    ...
    const cfList = (window.CustomFields && window.CustomFields.getFields(s, "classes")) || [];
    return ((window.APP.school?.classes) || []).map(c => {
      ...
      cfList.forEach(cf => {
        rowObj[cf.id] = (c.customValues && c.customValues[cf.id] != null) ? c.customValues[cf.id] : "";
      });
      return rowObj;
    });
  }
  ...
  function columns() {
    ...
    if (window.CustomFields) {
      cols.push(...window.CustomFields.getCustomColumns("classes"));
    }
    return cols;
  }
  ...
  function openEdit(r) {
    ...
    function save() {
      ...
      if (!isNew) {
        ...
        window.APP.mutate("Edit class", () => {
          ...
          c.customValues = Object.keys(draftCustomValues).length ? { ...draftCustomValues } : undefined;
    ...
    const cfInputs = (window.CustomFields && window.CustomFields.buildEditFields("classes", draftCustomValues)) || [];
  ...
  function open(opts) {
    ...
    D.open({
      ...
      onAction:(cmd, row) => {
        ...
        if (cmd === "customfields") {
          if (window.CustomFields) {
            window.CustomFields.open({ entity: "classes", onUpdate: () => D.refresh(rows(), columns()) });
          }
          return;
        }
        if (cmd === "delete" && row) {
...
export const EntityClasses = window.EntityClasses;
