...
(function (global) {
  ...
  function rows() {
    ...
    const cfList = (window.CustomFields && window.CustomFields.getFields(s, "classrooms")) || [];
    return ((window.APP.school?.classrooms) || []).map(rm => {
      ...
      cfList.forEach(cf => {
        rowObj[cf.id] = (rm.customValues && rm.customValues[cf.id] != null) ? rm.customValues[cf.id] : "";
      });
      return rowObj;
    });
  }
  ...
  function columns() {
    ...
    if (window.CustomFields) {
      cols.push(...window.CustomFields.getCustomColumns("classrooms"));
    }
    return cols;
  }
  ...
  function openEdit(r) {
    ...
    function save() {
      ...
      if (!isNew) {
        ...
        window.APP.mutate("Edit classroom", () => {
          ...
          rm.customValues = Object.keys(draftCustomValues).length ? { ...draftCustomValues } : undefined;
    ...
    const cfInputs = (window.CustomFields && window.CustomFields.buildEditFields("classrooms", draftCustomValues)) || [];
  ...
  function open(opts) {
    ...
    D.open({
      ...
      onAction:(cmd, row) => {
        ...
        if (cmd === "customfields") {
          if (window.CustomFields) {
            window.CustomFields.open({ entity: "classrooms", onUpdate: () => D.refresh(rows(), columns()) });
          }
          return;
        }
        if (cmd === "delete" && row) {
...
export const EntityClassrooms = window.EntityClassrooms;
