...
import "../components/teacher_constraints_dialog.js";
...
(function (global) {
  ...
  function rows() {
    ...
    const cfList = (window.CustomFields && window.CustomFields.getFields(s, "teachers")) || [];
    return ((window.APP.school?.teachers) || []).map(t => {
      ...
      cfList.forEach(cf => {
        rowObj[cf.id] = (t.customValues && t.customValues[cf.id] != null) ? t.customValues[cf.id] : "";
      });
      return rowObj;
    });
  }
  ...
  function columns() {
    ...
    if (window.CustomFields) {
      cols.push(...window.CustomFields.getCustomColumns("teachers"));
    }
    return cols;
  }
  ...
  function openEdit(r) {
    ...
    function save() {
      ...
      if (!isNew) {
        ...
        window.APP.mutate("Edit teacher", () => {
          ...
          t.customValues = Object.keys(draftCustomValues).length ? { ...draftCustomValues } : undefined;
    ...
    const cfInputs = (window.CustomFields && window.CustomFields.buildEditFields("teachers", draftCustomValues)) || [];
  ...
  function open(opts) {
    ...
    D.open({
      ...
      onAction:(cmd, row) => {
        ...
        if (cmd === "customfields") {
          if (window.CustomFields) {
            window.CustomFields.open({ entity: "teachers", onUpdate: () => D.refresh(rows(), columns()) });
          }
          return;
        }
        if (cmd === "delete" && row) {
...
export const EntityTeachers = window.EntityTeachers;
