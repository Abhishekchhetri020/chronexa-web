...
import "../state.js";
import "../components/help_tooltip.js";
...
(function (global) {
  ...
  // Shared option label formatter: shows "name (short)" only when short differs
  // from name (case-insensitive). Prevents "Mr. Zaid (Mr. Zaid)", "I A (I A)", "MATHS (Maths)".
  function formatOptionLabel(entOrName, maybeShort) {
    let name, short;
    if (entOrName && typeof entOrName === "object") {
      name = entOrName.name != null ? entOrName.name : "";
      short = entOrName.short != null ? entOrName.short : (entOrName.abbr != null ? entOrName.abbr : "");
    } else {
      name = entOrName != null ? entOrName : "";
      short = maybeShort != null ? maybeShort : "";
    }
    const sName = String(name).trim();
    const sShort = String(short).trim();
    if (!sShort) return sName;
    if (!sName) return sShort;
    if (sName.toLowerCase() === sShort.toLowerCase()) {
      return sName;
    }
    return `${sName} (${sShort})`;
  }
...
export const EntityDialog = window.EntityDialog;
export const formatOptionLabel = window.EntityDialog.formatOptionLabel;
