...
(function (global) {
  ...
  // B17 — the bottom-right toast must not paint over dialog footers (e.g.
  // Verification Pro). While any modal dialog is open the banner hides;
  // it restores itself when the last modal closes.
  const MODAL_SELECTOR = '.chrx-vpro-root, [role="dialog"], .chrx-modal, .modal.show, [data-modal-open="1"]';
  function isModalOpen() {
    try { return !!document.querySelector(MODAL_SELECTOR); } catch (_) { return false; }
  }
  function syncBannerWithModals() {
    const banner = document.getElementById("chrx-pwa-banner");
    const modal = isModalOpen();
    if (!banner) return modal;
    if (modal) {
      if (banner.dataset.modalHidden !== "1") {
        banner.dataset.modalHidden = "1";
        banner.style.display = "none";
      }
    } else if (banner.dataset.modalHidden === "1") {
      delete banner.dataset.modalHidden;
      banner.style.display = "";
    }
    return modal;
  }
  function watchModalsForBanner() {
    ...
    const start = () => {
      ...
      try {
        new MutationObserver(() => { try { syncBannerWithModals(); } catch (_) {} })
          .observe(body, {
            childList: true, subtree: true,
            attributes: true, attributeFilter: ["class", "style", "hidden", "open"],
          });
      } catch (_) {}
      try { syncBannerWithModals(); } catch (_) {}
    };
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", start, { once: true });
    } else start();
  }
  watchModalsForBanner();
  ...
  global.PWA = { installable, showBanner, isModalOpen, syncBannerWithModals };
})(window);

// [vite-esm] exports auto-generated by the 2026-07 Vite migration.
export const PWA = window.PWA;
