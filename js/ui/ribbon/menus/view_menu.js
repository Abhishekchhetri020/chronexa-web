...
import "../../components/saved_views.js";
import "../topbar.js";
...
(function () {
  ...
  // Custom saved views (Wave 3 Lane 2). Delegated to window.SavedViews
  // which persists onto APP.school.savedViews via APP.mutate.
  // Legacy localStorage helpers retained as fallback.
  ...
  function applyView(v) {
    if (!v) return;
    if (window.SavedViews && typeof window.SavedViews.apply === "function") return window.SavedViews.apply(v);
  ...
  function saveCurrentView() {
    if (window.SavedViews && typeof window.SavedViews.promptSave === "function") return window.SavedViews.promptSave();
  ...
  function deleteSavedView(name) {
    if (window.SavedViews && typeof window.SavedViews.delete === "function") return window.SavedViews.delete(name);
    const list = loadSaved().filter(v => v.name !== name);
    persistSaved(list);
  ...
  function getViewsList() {
    if (window.SavedViews && typeof window.SavedViews.list === "function") {
      const list = window.SavedViews.list();
  ...
  APP.ribbon.registerMenu({
    ...
    build() {
      ...
      return [
        ...
        { icon: "⚙️", label: "Manage saved views…", run: () => { if (window.SavedViews && window.SavedViews.openManager) window.SavedViews.openManager(); } },
        ...getViewsList().map(v => ({
          icon: "▶", label: v.name, run: () => applyView(v),
        })),
        ...(getViewsList().length ? [{ icon: "🗑", label: "Delete a saved view…",
          run: () => {
            const list = getViewsList();
