...
export function solve(school, options = {}) {
  ...
  for (let run = 0; run < maxRestarts; run++) {
    ...
    const ctx = {
      ...
      // this long, freeing the budget for repair/LNS. Enabled at ALL budgets:
      // measured on demo_sample-school.xml — best-of-6 @60s dropped 18 → 11
      // unplaced once the bail was forced on, because backtracking otherwise
      // plateaus and starves repair. Previously the bail auto-disabled above
      // a ~3s window (i.e. at every budget users actually run), which is
      // exactly where it helps most. Aggressive + capped (1500-2500ms) so
      // repair gets the slack early without bailing on still-productive runs.
      ...
      stagnationMs: options.stagnationMs != null
        ...
        : (useIterativeRepair
            ? Math.max(1500, Math.min(2500, timeLimitSec * 1000 * btShare * 0.1))
            : -1),
